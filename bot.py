...
user_bot = Bot(token=USER_TOKEN, default=DefaultBotProperties(parse_mode=ParseMode.HTML))
dp = Dispatcher()

logging.basicConfig(level=logging.INFO)

async def handle(request):
    return web.Response(text="BOTS ACTIVE")
...
# --- НОВОЕ: Обработка команды /start в админ-боте ---
@dp.message(Command("start"))
async def cmd_start(message: types.Message, bot: Bot):
    if bot.token == ADMIN_TOKEN:
        if message.from_user.id in ADMIN_IDS:
            await message.answer("<b>✅ СИСТЕМА ПОДКЛЮЧЕНА.\nТеперь эдиты будут приходить сюда автоматически!</b>")
        else:
            await message.answer("<b>❌ У вас нет доступа к этой панели.</b>")
    else:
        await message.answer("<b>🎬 Привет! Присылай свой эдит (видео или файл), и я отправлю его на модерацию.</b>")
...
async def handle_edit(message: types.Message, bot: Bot):
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            pass # Если админ не нажал старт, просто пропускаем
