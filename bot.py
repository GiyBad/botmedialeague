...
async def send_forward(from_chat_id, message_ids):
    try:
        ...
        await bot.send_message(from_chat_id, "✅ Опубликовано!")
...
async def set_day(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.message.edit_text(f"📅 Дата: {d}. Время (ЧЧ:ММ):")
...
# ГЛАВНЫЙ ЗАПУСК БЕЗ ПОТОКОВ
async def main():
    scheduler.start()
    await bot.delete_webhook(drop_pending_updates=True)
    
    # Запускаем бота и веб-сервер одновременно в одном цикле
